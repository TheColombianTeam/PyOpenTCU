...
class Tensor(FaultInjector):
    ...
    def mul(self, a, b, c):
        ...
        for instruction in range(len(HMMA_INTS)):
            ...
            for thread_group in range(self._thread_groups):
                self._fill_tensor_buffer(
                    thread_group, HMMA_INTS[instruction], instruction
                )
    ...
    def _fill_register_files(self, a, b, c):
        ...
        for thread_id in range(self._threads_per_warp):
            ...
            debug_print("Source1:\n {}\n".format(source))
            self._register_files[thread_id].fill(a, b, c, pointer, source, enable_c)
    ...
    def _init_registers(self):
        ...
        for idx in range(self._total_tensor_buffer):
            self._tensor_buffer.append(TensorBuffer(idx, fault_id=self.__fault_id))
            self._output_tensor_buffer.append(TensorBuffer())

    def _fill_tensor_buffer(
        self, thread_group, instruction_sources, instruction_number
    ):
        debug_print(
            "Starting filling tensor buffer {} thread group".format(thread_group)
        )
        ...
        for i in range(4):
            if instruction_number % 2 == 0:
                values = self._register_files[thread_group * 4 + i].rf_read(
                ...
                self._tensor_buffer[pointer_tensor_buffer].buffer_write(
                    "A", address, values[0], pointer_storage
                )
                address = "a_{}1".format(i)
                debug_print("Address {}".format(address))
                debug_print("Pointer tensor {}".format(pointer_tensor_buffer))
                self._tensor_buffer[pointer_tensor_buffer].buffer_write(
                    "A", address, values[1], pointer_storage
                )
                ...
                debug_print("Pointer tensor {}".format(pointer_tensor_buffer))
                self._tensor_buffer[pointer_tensor_buffer].buffer_write(
                    "B", address, values[0], pointer_storage
                )
                address = "b_1{}".format(i)
                debug_print("Address {}".format(address))
                debug_print("Pointer tensor {}".format(pointer_tensor_buffer))
                self._tensor_buffer[pointer_tensor_buffer].buffer_write(
                    "B", address, values[1], pointer_storage
                )
                ...
                debug_print("Pointer tensor {}".format(pointer_tensor_buffer))
                self._tensor_buffer[pointer_tensor_buffer].buffer_write(
                    "C", address, values[0], pointer_storage
                )
                address = "c_{}1".format(i)
                debug_print("Address {}".format(address))
                debug_print("Pointer tensor {}".format(pointer_tensor_buffer))
                self._tensor_buffer[pointer_tensor_buffer].buffer_write(
                    "C", address, values[1], pointer_storage
                )
    ...
    def _tensor_ele(self, a, b, c, n):
        ...
        if len(a) > n and len(b) > n:
            raise Exception("Error using the tensor element...")
    ...
    def _volta(self, thread_group, inst):
        ...
        for k in range(4):
            for i in range(4):
                for j in range(4):
                    A[k * 4 + i][j] = self._tensor_buffer[
                        pointer_tensor_buffer
                    ].read_buffer("A", "a_{}{}".format(k, j), pointer_a)
                    B[k * 4 + i][j] = self._tensor_buffer[
                        pointer_tensor_buffer
                    ].read_buffer("B", "b_{}{}".format(j, i), pointer_b)
                C[k * 4 + i] = self._tensor_buffer[pointer_tensor_buffer].read_buffer(
                    "C", "c_{}{}".format(k, i), pointer_c
                )
        ...
        for k in range(4):
            for i in range(4):
                W[k][i] = self._dot_product(
                    A[k * 4 + i], B[k * 4 + i], C[k * 4 + i], thread_group, k, i
                )
        ...
        for i in range(4):
            for j in range(4):
                if buffer_c_group == 0:
                    self._tensor_buffer[pointer_tensor_buffer].buffer_write(
                        "C", "c_{}{}".format(i, j), W[i][j], buffer_c_des
                    )
                elif buffer_c_group == 1:
                    self._tensor_buffer[pointer_tensor_buffer].buffer_write(
                        "CX", "c_{}{}".format(i, j), W[i][j], buffer_c_des
                    )
    ...
    def _fma_early_accumulate(self, a, b, c, thread_group, x, y):
        if self._type == "float16":
            ...
            dot_products = self.interconnection_fault_inject(
                dot_products, thread_group, x, y, 0
            )
            dot_products += Float16(a[1]) * Float16(b[1])
            dot_products = self.interconnection_fault_inject(
                dot_products, thread_group, x, y, 1
            )
            dot_products += Float16(a[2]) * Float16(b[2])
            dot_products = self.interconnection_fault_inject(
                dot_products, thread_group, x, y, 2
            )
            dot_products += Float16(a[3]) * Float16(b[3])
            dot_products = self.interconnection_fault_inject(
                dot_products, thread_group, x, y, 3
            )
            return dot_products
        else:
    ...
    def _tree_adds(self, a, b, c, thread_group, x, y):
        if self._type == "float16":
            ...
            for i in range(4):
                ...
                dot_products[i] = self.interconnection_fault_inject(
                    dot_products[i], thread_group, x, y, i
                )
            return (
                dot_products[0]
                + dot_products[1]
