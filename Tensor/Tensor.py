...
class TensorCore(object):
        ...
        for idx_inst, inst in enumerate(HMMA_INTS):
            for thread_group in range(self.__num_thread_groups):
                ...
                for k in range(4):
                    for i in range(4):
                        for j in range(4):
                            A[k * 4 + i][j] = self.__buffers[
                                pointer_tensor_buffer
                            ].read_buffer("A", "a_{}{}".format(k, j), pointer_a)
                            B[k * 4 + i][j] = self.__buffers[
                                pointer_tensor_buffer
                            ].read_buffer("B", "b_{}{}".format(j, i), pointer_b)
                        C[k * 4 + i] = self.__buffers[
                            pointer_tensor_buffer
                        ].read_buffer("C", "c_{}{}".format(k, i), pointer_c)

                # TODO: Parallelise using the number of TCU available
                ...
                for i in range(4):
                    for j in range(4):
                        self.__buffers[pointer_tensor_buffer].buffer_write(
                            buffer_c_group, "c_{}{}".format(i, j), D[i][j], buffer_c_des
                        )

                for i in range(4):
                    for j in range(2):
                        data = [D[i][j * 2], D[i][j * 2 + 1]]
                        self.__RFs[thread_group * 4 + i].rf_write(inst[0] + j, data)
        # TODO: include reading results
    ...
    def __fill_tensor_buffer(
        self, thread_group: int, inst_sources: List[int], inst: int
    ):
                ...
                for i in range(1, 4):
                    ...
                    for y in range(2):
                        ...
                        for z in range(2):
                            ...
                            self.__buffers[pointer_tensor_buffer].buffer_write(
                                addres_.upper(), address, values[z], pointer_storage
                            )
            else:
                for i in range(2):
                    values = self.__RFs[thread_group * 4 + op].rf_read(
                    ...
                    for y in range(2):
                        ...
                        self.__buffers[pointer_tensor_buffer].buffer_write(
                            "C", address, values[y], pointer_storage
                        )
        ...
        for thread_per_warp in range(self.__threads_per_warp):
            ...
            for i in range(4):
                ...
                self.__RFs[thread_per_warp].fill(
                    self.__a, self.__b, self.__c, pointer, source, enable_c
                )
